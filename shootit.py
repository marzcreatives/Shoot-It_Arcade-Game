    ...
    if i < 3:
        for j in range(1, 4):
            target_images[i - 1].append(pygame.transform.scale(
                pygame.image.load(f'assets/targets/{i}/{j}.png'), (110 - (j*18), 80 - (j*12))))
    else:
        for j in range(1, 5):
            target_images[i - 1].append(pygame.transform.scale(
    ...
    for i in range(max_val):
        for j in range(len(coords[i])):
            ...
            if my_coords[0] < -150:
                ...
                coords[i][j] = (my_coords[0] - 2**i, my_coords[1]) # makes the speed of each tier faster
    return coords
...
def draw_level(coords):
    if level == 1 or level == 2:
        ...
        target_rects = [[], [], [], []]  # rectangle hit boxes on the images to react to clicks
    for i in range(len(coords)):
        for j in range(len(coords[i])):
            target_rects[i].append(pygame.rect.Rect((coords[i][j][0] + 10, coords[i][j][1]),
                                                    (70 - i * 12, 70 - i * 12)))
    ...
    for i in range(len(targets)):
        for j in range(len(targets[i])):
            if targets[i][j].collidepoint(mouse_pos):
                coords[i].pop(j)
                points += 10 + 10 * (i**2)  # makes it 10, 20, 50, 100 points progressively
                if level == 1:
                    balloon_sound.play()
                elif level == 2:
                    cup_sound.play()
...
def draw_menu():
    global game_over, pause, mode, level, menu, time_passed, total_shots  #stops the need for passing and returning
    global  points, ammo, time_remaining, best_ammo, best_freeplay, best_timed, write_values, clicked, new_coords
    game_over = False
    pause = False
    screen.blit(menu_img, (0,0))
    mouse_pos = pygame.mouse.get_pos()
    clicks = pygame.mouse.get_pressed()
    freeplay_button = pygame.rect.Rect((170, 524), (260, 100)) #starting coords(x,y), (width, height)
    ...
    # freeplay_button = pygame.draw.rect(screen, [178, 524, 260, 100], 3) 3 = line weight, without = fully filled
    if freeplay_button.collidepoint(mouse_pos) and clicks[0] and not clicked: #prevents unwanted double click
        mode = 0
        level = 1
        menu = False
        time_passed = 0
        ...
        clicked = True
        new_coords = True
    if ammo_button.collidepoint(mouse_pos) and clicks[0] and not clicked: #prevents unwanted double click
        mode = 1
        level = 1
        menu = False
        time_passed = 0
        ...
        clicked = True
        new_coords = True
    if timed_button.collidepoint(mouse_pos) and clicks[0] and not clicked: #prevents unwanted double click
        mode = 2
        level = 1
        menu = False
        time_remaining = 30
...
def draw_game_over():
    global clicked,level, game_over, pause, menu, points, time_remaining, time_passed, total_shots
    if mode == 0:
...
def draw_pause():
    global level, pause, menu, points, time_remaining, time_passed, total_shots, clicked, new_coords
...
while run:
    ...
    if new_coords:
        ...
        three_coords = [[], [], [], []]
        for i in range(3):
            my_list = targets[1]  # how many enemies on the screen at the time, store their coordinates into my_List
            for j in range(my_list[i]):  # width divided by number of targets on the screen
                one_coords[i].append((WIDTH // (my_list[i]) * j, 300 - (i * 150) + 50 * (j % 2)))
        for i in range(3):
            my_list = targets[2]
            for j in range(my_list[i]):  # 30 * (j % 2) gets 'every other'
    ...
    if level == 1:
        ...
        if shot:
            ...
            shot = False  # prevents the shot being a laser gun dragging on screen (has to be clicked once per shot)
    elif level == 2:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1: #only if clicked with left-click
